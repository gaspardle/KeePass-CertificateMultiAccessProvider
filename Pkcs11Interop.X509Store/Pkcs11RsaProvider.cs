    ...
    public class Pkcs11RsaProvider : RSA
    {
		...
		public override byte[] Decrypt(byte[] data, RSAEncryptionPadding padding)
        {
            ...
            else if (padding.Mode == RSAEncryptionPaddingMode.Oaep)
            {
                ...
                var param = mechanismParamsFactory.CreateCkRsaPkcsOaepParams((ulong)hash_alg, (ulong)mgf, CKZ.CKZ_DATA_SPECIFIED, null);

                using (ISession session = _certContext.TokenContext.SlotContext.Slot.OpenSession(SessionType.ReadOnly))
                using (IMechanism mechanism = session.Factories.MechanismFactory.Create(CKM.CKM_RSA_PKCS_OAEP, param))
                {
                    return session.Decrypt(mechanism, _certContext.PrivKeyHandle, data);
                }
            }
